...
def _eta_from_waypoints(shipped_at: datetime | None, route_waypoints: list[dict], order_id: int) -> tuple[float, int]:
    ...
    if stop:
        ...
        total_dur = float(pickup.get("total_duration_seconds") or stop.get("duration_seconds") or 0)
        if shipped_at and total_dur > 0:
            ...
            elapsed = max(0.0, total_dur - max(0.0, eta_secs))
            progress = min(1.0, max(0.0, round(elapsed / total_dur, 4)))
        else:
